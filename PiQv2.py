...
#                    print("***** Warning: Failed to upload data to grovestreams. HTTP Failure Reason: " + response.reason + " body: " + response.read().decode(encoding="UTF-8"))
#                else:
#                    print("***** Warning: Failed to upload data to grovestreams. HTTP Failure Body: " + response.read().decode(encoding="UTF-8"))
#            except Exception:
#                print("***** Warning: Failed to upload data to grovestreams. HTTP Failure Reason: %s" % status)
#    
#    except Exception as e:
#        print("***** Warning: Failed to upload data to grovestreams. HTTP Failure Reason: %s" % str(e))
#    
#    finally:
#        if conn != None:
#            conn.close()   
...
""" % (FROM_EMAIL_ADDRESS, alertEmail, subject, message, currentTimestamp)
    ...
    # check for issues and report
        ...
        print("***** Warning: Failed to send email or text notification. Exception %s" % str(e))         
...
#        Print("***** Warning: Failed to upload data to dweet.io. HTTPError({0}): {1}]".format(httperror.errno, httperror.strerror))
#    except Exception as e:
#        print("***** Warning: Failed to upload data to dweet.io. Exception %s" % str(e))        
...
def turn_heat_on():
    try:
        print("\tRelay: Turning Fan On at %s" % datetime.datetime.now().time())
        GPIO.output(GPIO_RELAY_FAN_PIN, GPIO.LOW)        
    except Exception as e:
        print("***** Warning: Failed to interract with device (Relay On). Exception: %s" % str(e))
        raise
...
def turn_heat_off():
    try:
        print("\tRelay: Turning Fan Off at %s" % datetime.datetime.now().time())
        GPIO.output(GPIO_RELAY_FAN_PIN, GPIO.HIGH)        
    except Exception as e:
        print("***** Warning: Failed to interract with device (Relay Off). Exception: %s" % str(e))
        raise
...
def PID_Setup_Loop(grillSetupTemp, alertEmail, alertFrequency):
    ...
    print("Entering Setup Loop until we reach %0.2f degrees fahrenheit. Current temperature is %0.2f" % (grillSetupTemp, current_temp))
    ...
    while grillSetupTemp > current_temp:
        ...
        if heater_state == "off":
            heater_state = "on"
            print("Turning Fan ON until we reach %0.2f. Current temperature is %0.2f" % (grillSetupTemp, current_temp))
            turn_heat_on()
        else:
            print("Leaving the Fan ON until we reach %0.2f. Current temperature is %0.2f" % (grillSetupTemp, current_temp))
        sleep(10)
    ...
    if heater_state == "on":
        heater_state = "off"
        print("Turning Fan OFF since we reached or exceeded %0.2f. Current temperature is %0.2f" % (grillSetupTemp, current_temp))
        turn_heat_off()
...
def PID_Control_Loop(desiredGrillTemp, desiredMeatTemp, alertEmail, alertFrequency, loopInterval):
    ...
    print("Entering Control Loop with Grill [now=%0.2f, desired=%0.2f] -- Meat [now=%0.2f, desired=%0.2f]" % (currGrillTemp, desiredGrillTemp, currMeatTemp, desiredMeatTemp))
    ...
    while True:
        ...
        print("Readings: Grill [now=%0.2f, desired=%0.2f] -- Meat [now=%0.2f, desired=%0.2f] -- Fan for ~%d secs -- Time Left %0.2f mins" % (currGrillTemp, desiredGrillTemp, currMeatTemp, desiredMeatTemp, leaveTheFanOnTime, timeLeft))
        ...
        if difference < 0 and difference >= -3:
            ...
            print("We are close to our desired temperature for the Grill. Do Nothing!")
        elif difference < -3:
            ...
            if (elapsedTimeForNotification / 60) > alertFrequency:
                ...
                print("* Note: to avoid charcoal from extinguishing, run the fan for 3 seconds, even if our desired grill temperature has been exceeded")
                if heater_state == "off":
                    heater_state = "on"
                    turn_heat_on()
                else:
...
def main(argv):    
    if len(sys.argv) < 6:
        print("Usage: Raspberry-PI-Q.py [setup temperature to reach with continuous air] [grill temperature] [meat temperature] [alert email] [frequency of notifications in minutes] [loop interval in seconds; recommended 60] [unique name for your device; like Raspberry-PI-Q-Michael] [secret API key for grovestreams] &")
    ...
    try:
        ...
        print("One of the arguments was invalid")
        print("Usage: Raspberry-PI-Q.py [setup temperature to reach with continuous air] [grill temperature] [meat temperature] [alert email] [frequency of notifications in minutes] [loop interval in seconds; recommended 60] [unique name for your device; like Raspberry-PI-Q-Michael] [secret API key for grovestreams]")
        sys.exit(1)
    print("grillSetupTemp=%d, desiredGrillTemp=%d, desiredMeatTemp=%d, alertEmail=%s, alertFrequency=%d, loopInterval=%d," % (grillSetupTemp, desiredGrillTemp, desiredMeatTemp, alertEmail, alertFrequency, loopInterval,))
    ...
    try:        
        ...
        print("***** Alert: An exception occured in running the setup or control loops. Trying again. Error: %s" % excp)
        ...
        print("Running program again in 10 seconds")
        for x in range(1,10):
            print(x)
            sleep(1)
        main(sys.argv[1:])
